...
import { Plus, ChevronRight, BookOpen, Clock, Radio, User, SlidersHorizontal, CalendarDays, Users, Bell, X, RefreshCw, Check, Video, CheckCircle2, ExternalLink, Brain, Flame, Trophy } from "lucide-react";
import {
  DUMMY_OTHER_COURSES,
...
import { listOlympiads, olympiadStatus, useOlympiadState } from "../shared/olympiads";
import { useArenaState, getDivision } from "../shared/arena";
import { listEvents, eventStatus } from "../shared/arena-events";
import { OlympiadTag, CountdownInline } from "./olympiad-ui";
import {
...
const DUMMY_PURCHASED_CONTENT: Array<{
...
}> = [
  {
    ...
    accentColor: "#a78bfa",
    openPath: "/course-complete/piano-beginner-solo",
  },
  {
    id: "express-app",
    type: "app",
...
// Compact single-row entry into the Olympiad hub. Reflects live state +
// registered count so the Classes tab nudges students toward an active or
// upcoming event. Exported so the classes-v1 variant reuses the same entry.
export function OlympiadEntryBanner({ onClick }: { onClick: () => void }) {
  const { isRegistered } = useOlympiadState();
  const all = listOlympiads();
  const liveCount = all.filter((o) => olympiadStatus(o).isLive).length;
  const registeredCount = all.filter((o) => isRegistered(o.id)).length;

  const subtitle = liveCount > 0
    ? `${liveCount} live now · compete nationally`
    : registeredCount > 0
    ? `Registered for ${registeredCount} · compete live`
    : "Live contests · ranks & prizes";

  // Same card shell as the sibling ClassesGameTiles (neutral --card surface,
  // hairline border, same type scale + chevron) so it belongs to the page —
  // but as the section hero it gets a SOLID gold icon tile (vs the siblings'
  // tinted tiles) for clear hierarchy without going off-style.
  const accent = "var(--warning-500)";
  ...
  return (
    <motion.button
      ...
      style={{
        gap: 12, margin: "0 16px", padding: 8, borderRadius: 12, cursor: "pointer",
        width: "calc(100% - 32px)",
        backgroundColor: "var(--card)",
    ...
    >
      <div className="flex items-center justify-center shrink-0" style={{
        ...
        background: `linear-gradient(155deg, ${accent} 0%, var(--warning-700) 100%)`,
        boxShadow: `0 0 16px color-mix(in srgb, ${accent} 34%, transparent)`,
      }}>
        <Trophy size={22} style={{ color: "var(--white)" }} strokeWidth={2} />
      </div>
      <div className="flex flex-col" style={{ flex: 1, minWidth: 0, gap: 2 }}>
        <span className="flex items-center" style={{ gap: 8, fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--foreground)", lineHeight: 1.25 }}>
          Olympiads
          {live && <OlympiadTag label="Live now" variant="error" dot />}
        </span>
        <span className="truncate" style={{ fontSize: "var(--text-2xs)", color: "var(--muted-foreground)", letterSpacing: 0.2, lineHeight: 1.3 }}>{subtitle}</span>
      </div>
      <ChevronRight size={16} style={{ color: "var(--muted-foreground)", flexShrink: 0 }} />
    </motion.button>
  );
...
/** Arena hub entry. Adapts to the player's state so it's never confusing:
 *  • first-time (not joined) → an inviting "New · compete & win" pitch, no stats
 *  • established → your skill LEVEL + division (the returning-player pull). Level is
 *    the progression — it grows and makes questions harder. */
export function ArenaHubCard({ onClick }: { onClick: () => void }) {
  const { state } = useArenaState();
  const firstTime = !state.onboarded;
  const division = firstTime ? null : getDivision(state.divisionId);
  const liveCount = firstTime ? 0 : listEvents().filter((e) => e.format !== "sprint" && eventStatus(e).phase === "live").length;
  const tileColor = firstTime ? "var(--warning-500)" : "var(--primary-500)";
  return (
    <motion.button
      whileTap={{ scale: 0.98 }}
      type="button"
      ...
      className="flex items-center text-left"
      style={{
        gap: 12, margin: "0 16px", padding: 14, borderRadius: 16, cursor: "pointer",
        width: "calc(100% - 32px)",
        // Brand-tinted chrome so it reads active + tappable (not a dead grey row),
        // independent of the tier colour (Silver would otherwise wash it out).
        background: "linear-gradient(135deg, color-mix(in srgb, var(--primary-500) 16%, var(--card)) 0%, var(--card) 70%)",
        border: "0.5px solid color-mix(in srgb, var(--primary-500) 32%, var(--border))",
        fontFamily: "var(--font-family-inter)",
      }}
    >
      {/* Metallic tier medallion — polished even for Silver (matches the standing card). */}
      <div className="flex items-center justify-center shrink-0" style={{
        width: 46, height: 46, borderRadius: 12,
        background: `radial-gradient(circle at 50% 28%, color-mix(in srgb, ${tileColor} 82%, var(--white)) 0%, ${tileColor} 52%, color-mix(in srgb, ${tileColor} 56%, var(--black)) 100%)`,
        border: `1px solid color-mix(in srgb, ${tileColor} 45%, var(--white))`,
        boxShadow: `0 2px 12px color-mix(in srgb, ${tileColor} 34%, transparent)`,
      }}>
        <Trophy size={22} style={{ color: "var(--white)" }} strokeWidth={2} />
      </div>
      <div className="flex flex-col" style={{ flex: 1, minWidth: 0, gap: 4 }}>
        <span className="flex items-center" style={{ gap: 8, fontSize: "var(--text-base)", fontWeight: 700, color: "var(--foreground)", lineHeight: 1.2 }}>
          Arena
          {firstTime
            ? <OlympiadTag label="New" variant="success" />
            : liveCount > 0
              ? <OlympiadTag label={`${liveCount} live`} variant="error" dot />
              : null}
        </span>
        <span className="flex items-center truncate" style={{ gap: 6, fontSize: "var(--text-xs)", color: "var(--muted-foreground)" }}>
          {firstTime || !division
            ? "Contests, levels & real rewards"
            : `${division.label} · climb levels, win gems`}
        </span>
      </div>
      {/* Quiet chevron — the card's brand border + tap-scale already signal tappable. */}
  ...
  return (
    <div className="flex flex-col" style={{ height: "100%", backgroundColor: "var(--background)", overflow: "hidden" }}>
      ...
      <div className="flex-1 overflow-y-auto" style={{ paddingBottom: 8 }}>
        ...
        <div className="flex flex-col" style={{ gap: 8, paddingTop: 20, paddingBottom: 20 }}>
          <div className="flex items-center" style={{ paddingLeft: 16, paddingRight: 16, gap: 8 }}>
            ...
            <span style={{ fontSize: "var(--text-sm)", fontWeight: "var(--font-weight-medium)", color: "var(--muted-foreground)" }}>
              ({DUMMY_PREP_LIVE.length + DUMMY_SCHEDULE.length})
            </span>
          </div>
        ...
        <div className="flex flex-col" style={{ gap: 8, paddingBottom: 20 }}>
          <div className="flex items-center justify-between" style={{ paddingLeft: 16, paddingRight: 16 }}>
            <div className="flex items-center" style={{ gap: 8 }}>
              ...
              <span style={{ fontSize: "var(--text-sm)", fontWeight: "var(--font-weight-medium)", color: "var(--muted-foreground)" }}>
                ({(purchasedCampTrack ? 1 : 0) + (savedCrashClass ? DUMMY_CRASH_COURSE_INFO.subjects.length : 0) + DUMMY_PREP_CLASSROOMS.length + DUMMY_CLASSROOMS.length})
              </span>
            </div>
            <div className="flex items-center" style={{ gap: 8 }}>
              <button
                className="flex items-center justify-center"
                style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: "var(--card-bg-secondary)", border: "none", cursor: "pointer" }}
                aria-label="Filter classrooms"
              >
                <SlidersHorizontal size={16} style={{ color: "var(--muted-foreground)" }} />
              </button>
              <button
                className="flex items-center justify-center"
                style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: "var(--card-bg-secondary)", border: "none", cursor: "pointer" }}
          ...
          >
            ...
            {DUMMY_PREP_CLASSROOMS.map((room) => (
              <PrepClassroomCard key={room.id} item={room} onClick={() => navigate(`/learning-path?subject=${room.subjectId}&exam=${room.examId}`)} />
            ))}
        ...
        {/* Arena entry — the competitive hub (daily Sprint + weekend events).
            Shows the active track's tier + skill level so the daily habit pulls
